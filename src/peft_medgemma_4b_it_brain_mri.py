...
def fine_tune_medgemma_brain_mri(
        model,
        processor,
        data,
    ...
    """
    Fine-tunes the MedGemma model on the Orvile Brain MRI dataset.
    ...
    trainer = SFTTrainer(
        model=model,
        args=config_medgemma_4b_it_brain_mri.sft_args,
        train_dataset=data["train"],
        # eval_dataset=formatted_data["validation"].shuffle().select(range(10)),
        eval_dataset=data["validation"],
        peft_config=config_medgemma_4b_it_brain_mri.lora_config,
